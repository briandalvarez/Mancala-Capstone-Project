...
from mancala.mancala import GUIMatch
from mancala.ai_profiles import MinimaxUIWrapper
from mancala.mancala import HumanPlayer
...
class GUI:
    def __init__(self):
        ...
        self.ACTIVE_SCOREBOARD = pygame.transform.scale(
            self.ACTIVE_SCOREBOARD, (400, 360)
        )
        self.INACTIVE_SCOREBOARD = pygame.transform.scale(
            self.INACTIVE_SCOREBOARD, (400, 360)
        )
    ...
    def update_display(self, text):
        ...
        # Draw Display text
        displayed_text = self.PIXEL_FONT2.render(
            self.display_text, True, (255, 255, 255)
        )
    ...
    def draw_game_screen(self):
        ...
        self.WIN.blit(
            player_label,
            (self.player_scoreboard_pos[0] + 93, self.player_scoreboard_pos[1] + 118),
        )
        self.WIN.blit(
            ai_label, (self.ai_scoreboard_pos[0] + 140, self.ai_scoreboard_pos[1] + 110)
        )
    ...
    def draw_pit_stone_counts(self):
        for pit_index, (x, y) in self.stone_count_positions.items():
            ...
            text_surface = self.PIXEL_FONT.render(
                str(self.stone_count[pit_index]), True, (255, 255, 255)
            )
    ...
    def draw_scores(self):
        ...
        player_score_text = self.PIXEL_FONT2.render(
            str(self.player_score), True, (0, 0, 0)
        )
        player_score_rect = player_score_text.get_rect(
            center=(
                self.player_scoreboard_pos[0] + 272,
                self.player_scoreboard_pos[1] + 142,
            )
        )
    ...
    def draw_buttons(self):
        ...
        if self.isButtonsActive == False:
            for button in self.pit_buttons.values():
                pygame.draw.circle(
                    self.WIN, (0, 0, 137), button["center"], button["radius"]
                )
        elif self.isButtonsActive == True:
            for button in self.pit_buttons.values():
                pygame.draw.circle(
                    self.WIN, (204, 85, 0), button["center"], button["radius"]
                )

        # pygame.display.update()

    def update_scoreboard(self, newScores):
        if not isinstance(newScores, list) or len(newScores) != 14:
            raise ValueError("Expected a 14-element flat board array")
    ...
    def draw_welcome_screen(self):
        ...
        welcome_text = self.PIXEL_FONT4.render(
            "Welcome to Mancala!", True, (255, 255, 255)
        )
        self.WIN.blit(welcome_text, (280, 180))

        # Draw input box label
        input_label = self.PIXEL_FONT2.render(
            "Please input your name: ", True, (255, 255, 255)
        )
...
def main():
    ...
    while run:
        for event in pygame.event.get():
            ...
            # Game State 1: Welcome
            if game_state == "welcome":
                GameGUI.draw_welcome_screen()

                if event.type == pygame.KEYDOWN and is_input_box_active:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if GameGUI.enter_button.collidepoint(event.pos):
                        ...
                        match = GUIMatch(
                            gui=GameGUI,
                            player1=HumanPlayer(1, board, GameGUI.get_playername()),
                            player2=MinimaxUIWrapper(
                                gui=GameGUI, number=2, board=board
                            ),
                        )
                        GameGUI.update_scoreboard(match.board.board_array())

            # Game State 2: Playing the game
            elif game_state == "game":
                GameGUI.draw_game_screen()

                if GameGUI.isButtonsActive:
                    GameGUI.update_display("Please select your next move!")
